...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        List<Integer> periodDaysList = project.getTaskLists().stream()
                .filter(inProgress::contains)
                .flatMap(task -> task.getTasks().stream())
                .map(t -> t.getCreated())
                .map(locToPer -> Period.between(locToPer, LocalDate.now()))
                .map(perToInt -> Math.abs(perToInt.getDays()))
                .collect(toList());

        double average = IntStream.range(0, periodDaysList.size())
                .map(n -> periodDaysList.get(n))
                .mapToDouble(d -> d)
                .average()
                .getAsDouble();
        //Then
